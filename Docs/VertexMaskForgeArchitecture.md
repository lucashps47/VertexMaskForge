...
- **Dynamic preview is Source-Topology + Material-Slot/Bounding-Box(Local-space)/Directional-Normal(Local-space)/Curvature-only.** No Render Vertex Dynamic seam exists; no other generator has a Dynamic generation path; `DynamicLayerEvaluator`/`DynamicLayerBatchCompositor`/`FVertexMaskForgeInstanceResultStore` still have no production caller (the M16-K.6D-5 preview path bypasses all three — see §2). Dynamic Bounding Box does not support World Space or Unified Bounds (M16-K.6D-8B/8C-C). Dynamic Directional Normal does not support World Space (M16-K.6D-8D-B/8D-C).
