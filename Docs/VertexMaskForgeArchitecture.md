...
- `FVertexMaskForgeDynamicLayerStack` — has a real, editable UI section. As of M16-K.6B this includes generator-type assignment per layer (`None`/`Material Slot`, via `SetLayerMaskGeneratorType`/`ClearLayerMask`) — `Material Slot` is the only generator type exposed, matching the only generator with real Dynamic generation (see the table above). Parameter editing (`SetLayerMaskParams`) is still not exposed by any UI control. A M16-K.6C attempt to expose `FVertexMaskForgeMaterialSlotParams` (`SelectedSlotIndex`/`bInvert`) via an open-domain `SelectedSlotIndex` editor was audited and reverted before commit: `SelectedSlotIndex`'s valid domain is per-entry (`[0, WorkingMesh.MaterialSlotOptions.Num())`, the same mesh-specific asset data the legacy Material Slot picker already reads), but `DynamicLayerStack` is panel-global, so an open-domain editor could not distinguish a valid index from one that is out of range or meaningless for a different mesh — see [Decision Log ADR-010](VertexMaskForgeDecisionLog.md#adr-010--restrict-dynamic-material-slot-editing-to-single-mesh-selections) for the addressing policy this future UI must follow. No production composition call site reads the stack, generates anything, or feeds `WorkingColors`/preview — assignment is configuration-only.
